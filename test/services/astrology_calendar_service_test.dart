...
import '../../lib/services/astrology_calendar_service.dart';
  ...
  group('AstrologyCalendarService', () {
    ...
    group('getAllEvents', () {
      ...
      test('should return stored events when available', () async {
        ...
        final prefs = await SharedPreferences.getInstance();
        final eventsJson = json.encode(sampleEvents.map((e) => e.toJson()).toList());
    ...
    group('getEventsInRange', () {
      test('should return events within date range', () async {
        ...
        final events = await AstrologyCalendarService.getEventsInRange(startDate, endDate);
        ...
        for (final event in events) {
          expect(
            event.dateTime.isAfter(startDate.subtract(Duration(days: 2))),
            isTrue,
            reason: 'Event ${event.title} should be after start date with buffer'
          );
          expect(
            event.dateTime.isBefore(endDate.add(Duration(days: 2))),
            isTrue,
            reason: 'Event ${event.title} should be before end date with buffer'
          );
        }
      });
      ...
      test('should return events within the specified date range', () async {
        ...
        final endDate = DateTime.now().add(Duration(days: 5));

        final events = await AstrologyCalendarService.getEventsInRange(startDate, endDate);

        expect(events, isNotEmpty);
        expect(events.every((event) => event.dateTime.isAfter(startDate.subtract(Duration(days: 1))) && event.dateTime.isBefore(endDate.add(Duration(days: 1)))), isTrue);
      });
    });
    ...
    group('getUpcomingEvents', () {
      test('should return events for next 30 days by default', () async {
        ...
        for (final event in events) {
          expect(
            event.dateTime.isAfter(now.subtract(Duration(days: 1))),
            isTrue,
            reason: 'Upcoming event should be after now'
          );
          expect(
            event.dateTime.isBefore(thirtyDaysFromNow.add(Duration(days: 1))),
            isTrue,
      ...
      test('should handle custom days parameter', () async {
        const customDays = 7;
        final events = await AstrologyCalendarService.getUpcomingEvents(days: customDays);
        
        expect(events, isA<List<CelestialEvent>>());
        
        final now = DateTime.now();
        final customDaysFromNow = now.add(Duration(days: customDays));
        ...
        for (final event in events) {
          ...
          expect(
            event.dateTime.isBefore(customDaysFromNow.add(Duration(days: 1))),
            isTrue,
      ...
      test('should handle zero days parameter', () async {
        final events = await AstrologyCalendarService.getUpcomingEvents(days: 0);
        
        expect(events, isA<List<CelestialEvent>>());
      });

      test('should handle large days parameter', () async {
        final events = await AstrologyCalendarService.getUpcomingEvents(days: 365);
      ...
      test('should return events for the next 30 days by default', () async {
        ...
        expect(events, isNotEmpty);
        expect(events.every((event) => event.dateTime.isAfter(DateTime.now()) && event.dateTime.isBefore(DateTime.now().add(Duration(days: 30)))), isTrue);
      });
    });
    ...
    group('getTodaysEvents', () {
      test('should return events for today', () async {
        ...
        for (final event in events) {
          expect(
            event.dateTime.isAfter(startOfDay.subtract(Duration(days: 1))),
            isTrue,
            reason: 'Today event should be after start of day with buffer'
          );
          expect(
            event.dateTime.isBefore(endOfDay.add(Duration(days: 1))),
            isTrue,
      ...
      test('should return events for today', () async {
        ...
        expect(events, isNotEmpty);
        expect(events.every((event) => event.dateTime.isAfter(startOfDay) && event.dateTime.isBefore(endOfDay)), isTrue);
      });
    });
    ...
    group('getEventsForMonth', () {
      test('should return events for specific month', () async {
        ...
        final events = await AstrologyCalendarService.getEventsForMonth(testMonth);
        ...
        for (final event in events) {
          expect(
            event.dateTime.isAfter(startOfMonth.subtract(Duration(days: 1))),
            isTrue,
            reason: 'Monthly event should be after start of month with buffer'
          );
          expect(
            event.dateTime.isBefore(endOfMonth.add(Duration(days: 1))),
            isTrue,
            reason: 'Monthly event should be before end of month with buffer'
          );
        }
      });
      ...
      test('should handle February in leap year', () async {
        ...
        final events = await AstrologyCalendarService.getEventsForMonth(febLeapYear);
      ...
      test('should handle December (year boundary)', () async {
        ...
        final events = await AstrologyCalendarService.getEventsForMonth(december);
    ...
    group('addCustomEvent', () {
      test('should add custom event to existing events', () async {
        ...
        expect(
          updatedEvents.any((e) => e.id == 'custom_test_event'),
          isTrue,
      ...
      test('should handle duplicate event IDs', () async {
        ...
        final events = await AstrologyCalendarService.getAllEvents();
        final duplicateEvents = events.where((e) => e.id == 'duplicate_id').toList();
    ...
    group('removeEvent', () {
      test('should remove event by ID', () async {
        ...
        expect(
          eventsWithAdded.any((e) => e.id == 'removable_event'),
          isTrue,
        ...
        final eventsAfterRemoval = await AstrologyCalendarService.getAllEvents();
        expect(
          eventsAfterRemoval.any((e) => e.id == 'removable_event'),
          isFalse,
      ...
      test('should remove all instances of duplicate IDs', () async {
        ...
        final events = await AstrologyCalendarService.getAllEvents();
        expect(
          events.any((e) => e.id == 'duplicate_remove_id'),
          isFalse,
    ...
    group('needsUpdate', () {
      ...
      test('should return false when last update is recent', () async {
        ...
        await prefs.setString('events_last_update', recentDate.toIso8601String());
      ...
      test('should handle exactly 7 days threshold', () async {
        final prefs = await SharedPreferences.getInstance();
        final exactlySevenDays = DateTime.now().subtract(Duration(days: 7));
        await prefs.setString('events_last_update', exactlySevenDays.toIso8601String());
    ...
    group('getEventsByType', () {
      test('should filter events by type', () async {
        ...
        final newMoonEvents = await AstrologyCalendarService.getEventsByType('new_moon');
        final retrogradeEvents = await AstrologyCalendarService.getEventsByType('retrograde');
      ...
      test('should return empty list for non-existent type', () async {
        final events = await AstrologyCalendarService.getEventsByType('non_existent_type');
        
        expect(events, isEmpty);
      });

      test('should handle case sensitivity', () async {
        final events = await AstrologyCalendarService.getEventsByType('NEW_MOON');
    ...
    group('getImportantEvents', () {
      test('should return only important events', () async {
        ...
        final importantEvents = await AstrologyCalendarService.getImportantEvents();
    ...
    group('searchEvents', () {
      ...
      test('should handle case insensitive search', () async {
        ...
        final upperResults = await AstrologyCalendarService.searchEvents('uppercase');
        final lowerResults = await AstrologyCalendarService.searchEvents('LOWERCASE');
      ...
      test('should return empty list for no matches', () async {
        final results = await AstrologyCalendarService.searchEvents('NonExistentSearchTerm');
    ...
    group('Sample Events Generation', () {
      test('should generate events for multiple months', () async {
        ...
        final futureEvents = events.where((e) => 
          e.dateTime.isAfter(now) && e.dateTime.isBefore(sixMonthsFromNow)
        ).toList();
    ...
    group('Storage and Persistence', () {
      ...
      test('should update last update timestamp when saving', () async {
        ...
        expect(lastUpdate.difference(DateTime.now()).abs().inMinutes, lessThan(1));
      });
    });
