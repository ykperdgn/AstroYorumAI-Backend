...
void main() {
  group('BirthInfoScreen Debug Tests', () {
    ...
    Widget createTestWidget(Widget child) {
      ...
      // Fill form with invalid latitude - but include ALL required fields
      await tester.enterText(find.byType(TextFormField).at(0), 'Test User');
      
      // Select date
      await tester.tap(find.text('Tarih Seç'));
      await tester.pump();
      await tester.tap(find.text('OK')); // DatePicker OK button
      await tester.pump();
      
      // Select time
      await tester.tap(find.text('Saat Seç'));
      await tester.pump();
      await tester.tap(find.text('OK')); // TimePicker OK button  
      await tester.pump();
      ...
      await tester.pump(Duration(milliseconds: 100)); // Allow time for validation
    ...
    testWidgets('DEBUG: What happens when submitting without date?', (WidgetTester tester) async {
      ...
      // Check for expected SnackBar message
      final snackBarMessage = find.text('Lütfen doğum tarihinizi seçin.');
      print('SnackBar message found: ${snackBarMessage.evaluate().length}');
    });
  });
}
