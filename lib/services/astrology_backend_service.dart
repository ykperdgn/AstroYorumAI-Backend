...
class AstrologyBackendService {  
  ...
  static Map<String, dynamic> _convertToTurkish(Map<String, dynamic> apiResponse) {
    ...
    if (convertedResponse.containsKey('planets')) {
      final originalPlanets = convertedResponse['planets'] as Map<String, dynamic>;
    ...
    if (convertedResponse.containsKey('ascendant')) {
      ...
      final turkishAscendant = _signNamesTurkish[originalAscendant] ?? originalAscendant;
  ...
  static Future<bool> checkHealth() async {
    ...
    final url = Uri.parse('$_baseUrl/natal');    try {
      ...
      return {
        "error": "Backend API Error: ${response.statusCode}",        "details": response.body,
        "url": url.toString()
      };
    } catch (e) {
      ...
      if (e.toString().contains('XMLHttpRequest error') || 
          e.toString().contains('CORS') ||          e.toString().contains('Access to XMLHttpRequest') ||
        ...
        userFriendlyMessage = "Web tarayıcısı güvenlik politikaları bu isteği engelledi. Bu Flutter web uygulamasının bilinen bir sınırlamasıdır.";
        specificSolution = "1. Chrome'u CORS disabled modda açın\n2. Veya mobil uygulamayı kullanın\n3. Veya Firefox tarayıcısını deneyin";
      } else if (e.toString().contains('SocketException') || 
                 e.toString().contains('ClientException') ||
                 e.toString().contains('Failed host lookup')) {
        errorMessage = "Ağ bağlantı hatası";
        userFriendlyMessage = "İnternet bağlantınızı kontrol edin. Sunucuya erişilemiyor.";
        specificSolution = "İnternet bağlantınızı kontrol edin ve tekrar deneyin";
      } else if (e.toString().contains('TimeoutException') ||
                 e.toString().contains('Connection timeout')) {
        errorMessage = "Bağlantı zaman aşımı";
        userFriendlyMessage = "Sunucu yanıt vermedi. Sunucu yoğunluğu olabilir.";
        specificSolution = "Birkaç dakika bekleyip tekrar deneyin";
      } else if (e.toString().contains('FormatException') ||
                 e.toString().contains('JSON')) {
        errorMessage = "Veri format hatası";
        userFriendlyMessage = "Sunucudan gelen yanıt bozuk. Sunucu sorunu olabilir.";
        specificSolution = "Daha sonra tekrar deneyin";
      } else {
        userFriendlyMessage = "Backend servisine bağlanılamıyor. Teknik sorun olabilir.";
