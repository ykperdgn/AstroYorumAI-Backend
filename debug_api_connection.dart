...
void main() async {
  ...
  const String apiUrl = 'https://astroyorumai-api.onrender.com';    // Test 1: Health check
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (result.containsKey('error')) {
        ...
        developer.log('   Planets: ${result['planets'] != null ? "Present" : "Missing"}');
      }
    } else {
      final responseBody = await response.transform(utf8.decoder).join();
      developer.log('❌ Flutter Style Error: ${response.statusCode} - $responseBody');
    }
    client.close();
  } catch (e) {
